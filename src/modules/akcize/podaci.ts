...
import { granicePerioda, obracunajAkcizu, rokPolumesecni, rokIstekao } from "@/lib/akciza";
...
export async function ucitajPeriode(): Promise<PeriodRed[]> {
  // Osnov akcize = PUNJENJE u boce (flaširanje), grupisano po polumesečnom periodu.
  const agg = (await db.execute(sql`
    select extract(year from datum)::int as godina,
           extract(month from datum)::int as mesec,
           (case when extract(day from datum) <= 15 then 1 else 2 end)::int as deo,
           coalesce(sum(cist_alkohol_l), 0)::float8 as litara
    from punjenje
    where proizvod_id is not null and cist_alkohol_l is not null and datum is not null
    group by 1, 2, 3
    order by 1 desc, 2 desc, 3 desc
  `)) as unknown as Array<{ godina: number; mesec: number; deo: number; litara: number }>;

  const [stope, obracuni] = await Promise.all([
