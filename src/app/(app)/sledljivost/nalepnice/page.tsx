...
export default async function NalepniceStranica() {
  ...
  return (
    <div>
      ...
      {nalepnice.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-neutral-300 bg-white py-16 text-center text-sm text-neutral-400 print:hidden">
          {sr.forma.nemaUnosa}
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 print:grid-cols-3">
          {nalepnice.map((n) => (
            <div
              key={n.id}
              className="flex break-inside-avoid flex-col items-center rounded-2xl border border-neutral-300 bg-white p-4 text-center"
            >
